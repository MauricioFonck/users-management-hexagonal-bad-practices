...
public final class CreateUserService implements CreateUserUseCase {
  ...
  private void ensureEmailIsAvailable(final UserEmail email) {
    if (getUserByEmailPort.getByEmail(email).isPresent()) {
      throw UserAlreadyExistsException.becauseEmailAlreadyExists(email.value());
    }
  }

  private void notifyWelcome(final UserModel user, final String plainPassword) {
    emailNotificationService.notifyUserCreated(user, plainPassword);
  }
}
